...
def test_successful_register(new_users_data):
    ...
    with allure.step(f'Делаем запрос по адресу {BASE_URL + REGISTER_USER}'):
        response = httpx.post(BASE_URL + REGISTER_USER, json=new_users_data, headers=headers)
...
@allure.suite('Проверка запросов списка пользователей')
@allure.title('Проверка таймаута у запроса списка пользователей')
